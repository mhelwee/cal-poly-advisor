"""Shared advising core: ONE canonical copy of the system prompt and the roadmap
generate -> validate -> regenerate loop, imported by both the web app (app.py) and the
terminal app (advisor.py) so the two prompts can't drift and the terminal reaches parity
with the web app.

No Flask and no Anthropic client are created here — the caller passes its own `client`
into generate_validated_reply, so this module stays a pure, importable core.
"""
...
def build_system_prompt(today, next_term):
    """Build the base system prompt. Query-specific context comes from RAG."""
...
#
# The model is asked (per message, so the system prompt stays untouched) to append a
# machine-readable <roadmap-json> block whenever it produces a multi-term plan. We parse
# that block, run validate_roadmap on it, and if it has violations we feed them back and
# ask the model to fix the plan — capped at MAX_ROADMAP_RETRIES so a stubborn model can't
# loop or run up cost. The block is stripped before the student ever sees it.
...
# Appended to the student-facing reply when the validation loop ends with a roadmap that
# still has violations (rare retry-exhaustion case), so we never present it as fully verified.
INCOMPLETE_VALIDATION_NOTE = (
    "\n\n_Note: I couldn't fully verify this plan against all prerequisite and term rules — "
    "please confirm specifics with your academic advisor._"
)


def first_text(response):
    """Return the text of the first text block in an Anthropic response, or "" if none.

    Doesn't assume content[0] is a text block (it may be a tool-use block, etc.).
    """
...
def _system_param(system_prompt):
    """Wrap the system prompt with a cache_control marker. The advising knowledge base is
    large and static within a day, so caching it cuts per-message cost and latency; it only
    misses once a day when the embedded date rolls over."""
...
def _roadmap_cs_cap(roadmap):
    """The per-term CS cap the model parsed from the student, or None. Tolerates the field
    being absent, null, or malformed (backward compatible)."""
...
def generate_validated_reply(client, messages, system_prompt, *, earliest_term, max_tokens=4096):
    """Generate a reply, validating any roadmap it emits and asking the model to fix
    violations (up to MAX_ROADMAP_RETRIES).

    Returns (final_reply, remaining_violations): remaining_violations is the validation
    result on the FINAL reply's roadmap — [] when the plan is clean or when there is no
    roadmap block. The reply still contains the raw <roadmap-json> block; the caller is
    responsible for stripping it with _strip_roadmap_block before display.

    Raises anthropic.APIError subclasses on API failure for the caller to handle, so a
    failure on any attempt (initial or retry) can degrade gracefully.
    """
    system = _system_param(system_prompt)
    response = client.messages.create(
        model=MODEL, max_tokens=max_tokens, system=system, messages=messages,
    )
    reply = first_text(response)
    convo = list(messages)
    ...
    for _ in range(MAX_ROADMAP_RETRIES):
        ...
        violations = validate_roadmap(
            roadmap, cs_cap=_roadmap_cs_cap(roadmap), earliest_term=earliest_term
        )
        if not violations:
            break
    ...
    # Final validation on whatever reply we ended with: covers the retry-exhaustion case
    # so the caller can flag a plan we could not fully verify.
    final_roadmap = _extract_roadmap(reply)
    remaining = (
        validate_roadmap(
            final_roadmap, cs_cap=_roadmap_cs_cap(final_roadmap), earliest_term=earliest_term
        )
        if final_roadmap is not None
        else []
    )
